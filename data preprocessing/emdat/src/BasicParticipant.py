...
def read_participants_Basic(datadir, user_list, pids, prune_length=None, aoifile=None,
    ...
    for rec, pid, offset in zip(user_list, pids, log_time_offsets):
        ...
        if params.EYETRACKERTYPE == "TobiiV2":
            ...
            allfile = "{dir}/Recovery_Participant24_{rec}.tsv".format(dir=datadir, rec=rec)
            fixfile = "{dir}/Recovery_Participant24_{rec}.tsv".format(dir=datadir, rec=rec)
            sacfile = "{dir}/Recovery_Participant24_{rec}.tsv".format(dir=datadir, rec=rec)
            #evefile = "{dir}/P24_{rec}_dynamicaoi.aoi".format(dir=datadir, rec=rec)
            evefile = None
            segfile = "{dir}/Recovery_Participant24_{targ}_{rec}_Window_{win}.seg".format(dir=datadir, targ=params.TARGET, rec=rec, win=params.WINDOW)
            #if aoifile and "static" not in aoifile:
            #    aoifile = "{diraoi}/P24_{rec:02}_dynamicaoi.aoi".format(diraoi=params.AOI, rec=rec)
        elif params.EYETRACKERTYPE == "SMI":
