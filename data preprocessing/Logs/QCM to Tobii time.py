...
end_positions_qcm_2024 = [51,75,103,134,162,179,189,199,213,225,335,313,341,236,402]
...
for i in uids:
    ...
    for j in range(len(start_positions_qcm_2024)):
        ...
        temp_df1 = df.loc[df[start_pos_col] == nearest_pos].copy()
        temp_df1["seg"] = str(j+1)
        temp_df1["start_end"] = "start"
        temp_df1["pos"] = (start_positions_qcm_2024[j] - nearest_pos) * 1000000 + temp_df1[starttobii_col]
        
        nearest_pos = find_nearest_pos_in_panopto(ref_start_positions, end_positions_qcm_2024[j])
        temp_df2 = df.loc[df[start_pos_col] == nearest_pos].copy()
        temp_df2["seg"] = str(j+1)
        temp_df2["start_end"] = "end"
        temp_df2["pos"] = (end_positions_qcm_2024[j] - nearest_pos) * 1000000 + temp_df2[starttobii_col]
    ...
    for j in range(len(start_positions_selfattention_2024)):
        ...
        temp_df1["seg"] = str(j+1)
        temp_df1["start_end"] = "start"
        temp_df1["pos"] = (start_positions_selfattention_2024[j] - nearest_pos) * 1000000 + temp_df1[starttobii_col]
        
        nearest_pos = find_nearest_pos_in_panopto(ref_start_positions, end_positions_selfattention_2024[j])
        temp_df2 = df.loc[df[start_pos_col] == nearest_pos].copy()
        temp_df2["seg"] = str(j+1)
        temp_df2["start_end"] = "end"
        temp_df2["pos"] = (end_positions_selfattention_2024[j] - nearest_pos) * 1000000 + temp_df2[starttobii_col]
